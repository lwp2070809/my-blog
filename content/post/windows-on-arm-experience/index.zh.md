...
非常满意的一点. 这颗低频版`Snapdragon 8cx Gen 3 @ 2.69 GHz`处理器的能耗表现在同期处理器中非常的厉害, 并且离电性能也可圈可点. 在Windows 11, 省电模式, 能源之星启用的情况下, 以仅仅40Wh的电池, 可以达到中度办公续航8小时的体验, 并且不会感觉到明显卡顿. 无论是此前`Surface Pro 7`的`i5-1035G4`, 还是后来新公司给我的thinkbook笔记本电脑搭载的`i5-1135G7`都无法达到这样的效果. 以thinkbook笔记本电脑为例, 搭载了57Wh的电池在同样的条件下却仅仅只有4小时的续航, 简直难以置信.
...
我没有测试过steam游戏, 根据网上看到的测评, 是可以运行很多游戏的. 由于我是Xbox用户, 并且我不想在这台电脑有限的存储空间中安装太多软件, 因此只选择Microsoft Store和Xbox PC的游戏. 在2025年8月之前, 只能在Microsoft Store安装游戏. 微软八件套中的部分游戏如纸牌合集等是原生ARM64, 可以下载游玩. 需要注意`Minecraft Windows Edition`虽然标注了ARM, 实际上是ARM32架构, 在Windows 11 24h2彻底移除了ARM32的支持后是无法游玩的.

2025年8月之后, Xbox PC在insider通道加入了对WoA本地游戏的支持(此前即使安装了Xbox PC也只能玩云游戏), 需要最新版本的Windows 11, 并在Xbox Insider Hub加入Xbox PC beta测试. 目前我已经在这台电脑上游玩过`星露谷物语`, `火山的女儿`, 并通关了`逆转裁判: 成步堂三部曲`和`深空梦里人2: 逐星之旅`. 由于只能通过兼容模式运行游戏并且高通的GPU驱动也不算太好, 因此在游戏中WoA的能耗就相当一般了. 但借助触摸屏, 对于部分只需要点击的文字游戏还是能做到比较独特的体验, 我挺喜欢在出差时躺床上捧着这台二合一平板电脑玩游戏.

## 一些踩坑

1. 和传统x64 PC的设计存在区别(内部似乎是Device Tree), UEFI的行为存在差异. 重装系统时可能碰到预期之外的问题, RTC精度低.
2. 由于RTC的问题导致这台电脑的时间非常不准, 我的缓解方案是使用win32tm每隔2-4小时就同步一次时间, 而非默认的7天, 并在Windows计划任务中添加一个联网即同步时间的任务, 使得电脑从睡眠中唤醒之后可以确保和真实时间一致.
3. 一些比较底层的老旧音频库不兼容ARM64.
4. 一些比较老的USB网卡不兼容.
5. 一些涉及到底层驱动的应用或功能不兼容, 如UU加速器的主机加速模式需要创建网卡, 在这台电脑上无法正常使用.

## 硬件和OEM

这部分和`WoA`的体验关系不是特别大, 因此移到最后. 
